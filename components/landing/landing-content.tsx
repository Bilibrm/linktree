...
const waypoints = [
  {
    ...
    desc: "No email verification. No credit card. Just a name and you are live.",
  },
  {
    id: "customize",
    label: "02",
    title: "Choose your look",
    desc: "Eight built-in themes. Every color, font, background — all yours to change.",
  },
  {
    id: "publish",
    label: "03",
    title: "Drop in your blocks",
    desc: "Links, images, embeds, forms, countdowns. Ten block types. Drag to reorder.",
  },
  {
    id: "grow",
    label: "04",
    title: "Watch what happens",
    desc: "Click tracking, views, referrers. Real numbers. No guesswork.",
  },
]
...
const faqs = [
  {
    q: "Is LinkNest free?",
    a: "Yes. Every feature is free. No premium tier, no paywall. We fund development through optional donations.",
  },
  {
    q: "Can I use my own domain?",
    a: "Not yet. Custom domain support is on the roadmap. Right now your page lives at linknest.app/yourusername.",
  },
  {
    q: "Do you track my visitors?",
    a: "We track click events and page views — numbers, not people. No cookies, no PII, no cross-site tracking. Analytics are aggregate only.",
  },
  {
    q: "Can I export my data?",
    a: "Yes. Your data is yours. Export your blocks and settings from the dashboard at any time.",
  },
  {
    q: "What block types are available?",
    a: "Links, headers, text, dividers, social icons, images, galleries, embeds (YouTube, Spotify, TikTok), forms, and countdowns.",
  },
  {
    q: "Is this open source?",
    a: "Yes. The full codebase is public. You can self-host, contribute, or just read the code.",
  },
]
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      mm.add("(min-width: 768px)", () => {
        const topoLayers = [topo1Ref.current, topo2Ref.current].filter(Boolean)
        topoLayers.forEach((layer, i) => {
          ScrollTrigger.create({
            ...
            onUpdate: (self) => {
              if (layer) {
                const y = self.progress * -(120 + i * 80)
                gsap.set(layer, { y, willChange: "transform" })
              }
            },
          })
        })
          ...
          ScrollTrigger.create({
            ...
            onUpdate: (self) => {
              ...
              if (marker) {
                ...
                const bg = gsap.utils.interpolate("#17332C", "#D2A24C", progress)
                const scale = gsap.utils.interpolate(1, 1.35, progress)
                gsap.set(marker, { backgroundColor: bg, scale })
              }
            },
            onLeave: () => {
              const marker = markerRefs.current[i]
        ...
        gsap.utils.toArray<HTMLElement>(".block-preview").forEach((el, i) => {
          ...
          ScrollTrigger.create({
            ...
            scrub: 1.5,
            onUpdate: (self) => {
              const rot = gsap.utils.interpolate(angle, 0, self.progress)
              gsap.set(el, { rotate: rot, y: gsap.utils.interpolate(40, 0, self.progress), opacity: gsap.utils.interpolate(0.3, 1, self.progress) })
            },
          })
        })
      })
      ...
      mm.add("(max-width: 767px)", () => {
        gsap.utils.toArray<HTMLElement>(".waypoint-card").forEach((card, i) => {
          ScrollTrigger.create({
            ...
            start: "top 85%",
            onEnter: () => {
              gsap.to(card, { y: 0, opacity: 1, duration: 0.6, delay: i * 0.15, ease, clearProps: "y" })
            },
            once: true,
          })
        })
      ...
      gsap.utils.toArray<HTMLElement>(".reveal").forEach((el) => {
        ScrollTrigger.create({
          ...
          start: "top 82%",
          onEnter: () => {
            gsap.to(el, { y: 0, opacity: 1, duration: 0.7, ease, clearProps: "y" })
          },
          once: true,
        })
      })
        ...
        onUpdate: (self) => {
          if (navRef.current) {
            if (self.progress > 0.03) {
              gsap.to(navRef.current, { y: 0, opacity: 1, duration: 0.3, overwrite: "auto" })
            } else {
              gsap.to(navRef.current, { y: -20, opacity: 0, duration: 0.3, overwrite: "auto" })
            }
          }
        },
      })
    ...
    <div ref={container} className="relative bg-ink text-bone font-body overflow-x-hidden min-h-screen">
      ...
      <div className="fixed top-0 left-0 h-full w-16 md:w-20 z-30 pointer-events-none hidden md:block will-change-transform">
        <svg className="w-full h-full" viewBox="0 0 80 2000" preserveAspectRatio="xMidYMax slice">
          <path
            ref={trailRef}
            d="M40,0 Q60,200 40,400 T40,800 T40,1200 T40,1600 T40,2000"
            fill="none"
            stroke="#D2A24C"
            strokeWidth="1.5"
            strokeLinecap="round"
        ...
        <div className="absolute top-0 left-1/2 -translate-x-1/2 h-full flex flex-col items-center gap-0">
          {waypoints.map((w, i) => (
            <div
              key={w.id}
              ref={setMarkerRef(i)}
              className="w-3 h-3 rounded-full border-2 border-gold/60"
              style={{ backgroundColor: "#17332C", marginTop: i === 0 ? "18vh" : "0", flex: "1", minHeight: "4rem" }}
            />
          ))}
        </div>
      </div>
      ...
      <section ref={heroRef} id="hero" className="relative min-h-screen flex items-center justify-center px-6 pt-24 md:pt-0 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-b from-transparent via-ink/80 to-ink pointer-events-none z-10" />
        <div
          ref={cursorRef}
          className="absolute w-6 h-6 rounded-full border-2 border-gold/20 pointer-events-none z-20 hidden md:block"
          style={{ left: "50%", top: "40%" }}
        />
        <div
          ref={cursorDotRef}
          className="absolute w-1.5 h-1.5 rounded-full bg-gold/30 pointer-events-none z-20 hidden md:block"
          style={{ left: "50%", top: "40%" }}
        />
        <div className="relative z-20 text-center max-w-3xl mx-auto">
          ...
          <h1 className="hero-line font-display text-display-xl md:text-[clamp(4rem,10vw,8rem)] font-black text-bone leading-[0.88]" style={{ perspective: 600 }}>
            Your trail
            <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-gold to-coral">starts here</span>
          </h1>
          <p className="hero-desc mt-6 text-body text-muted-foreground max-w-lg mx-auto leading-relaxed">
          ...
          <div className="hero-actions mt-10 flex items-center justify-center gap-3 flex-wrap">
            <Link href="/auth/signup" className="group inline-flex items-center gap-2 rounded-full bg-gold text-ink px-6 py-3 text-caption font-semibold hover:opacity-90 transition-all active:scale-[0.97]">
          ...
          <div className="space-y-20 md:space-y-32">
            {waypoints.map((w, i) => {
              ...
              return (
                <div
                  key={w.id}
                  ref={setWaypointSectionRef(i)}
                  className={`waypoint-section-${i} relative`}
                >
                  <div
                    className={`waypoint-card waypoint-card-${i}`}
                  >
      ...
      {/* BLOCK SHOWCASE */}
      <section className="relative px-6 py-32 md:py-48 max-w-5xl mx-auto overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-surface/20 to-transparent pointer-events-none" />
        <div className="relative reveal" style={{ y: 40, opacity: 0 }}>
          <span className="text-small uppercase tracking-widest font-semibold text-gold mb-4 block">Block showcase</span>
          <h2 className="font-display text-display-lg font-black text-bone leading-[1.05] mb-8 md:mb-16">What a page actually looks like.</h2>
        </div>
        <div className="relative grid sm:grid-cols-2 gap-5 mt-8">
          <div className="block-preview rounded-xl border border-white/5 bg-surface/50 p-5 md:p-6" style={{ rotate: "-1.5deg" }}>
            <span className="text-small font-mono text-muted-foreground mb-3 block">Link block</span>
            <div className="w-full h-12 rounded-xl bg-gold text-ink flex items-center px-5 font-semibold text-caption">
              Check out my latest project
            </div>
            <p className="text-small text-muted-foreground/60 mt-3 font-mono truncate">linknest.app/yourname/latest-project</p>
          </div>
          <div className="block-preview rounded-xl border border-white/5 bg-surface/50 p-5 md:p-6" style={{ rotate: "2deg", marginTop: "2rem" }}>
            <span className="text-small font-mono text-muted-foreground mb-3 block">Social icons</span>
            <div className="flex gap-3">
              {["ig", "x", "yt", "tt", "gh"].map((s) => (
                <div key={s} className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center text-small font-mono text-muted-foreground hover:border-gold/30 hover:text-gold transition-colors">
                  {s}
                </div>
              ))}
            </div>
          </div>
          <div className="block-preview rounded-xl border border-white/5 bg-surface/50 p-5 md:p-6" style={{ rotate: "1.5deg", marginLeft: "1rem" }}>
            <span className="text-small font-mono text-muted-foreground mb-3 block">Embed</span>
          ...
          <div className="block-preview rounded-xl border border-white/5 bg-surface/50 p-5 md:p-6" style={{ rotate: "-1deg", marginTop: "1rem" }}>
      ...
      <section className="relative px-6 py-32 md:py-48 max-w-5xl mx-auto">
        <div className="reveal" style={{ y: 40, opacity: 0 }}>
          ...
          <h2 className="font-display text-display-lg font-black text-bone leading-[1.05] mb-12">Pick a look.</h2>
          <div className="theme-grid flex flex-wrap gap-3 mb-10">
            {[
          ...
          <div
            className="rounded-2xl border border-white/5 p-8 md:p-12 transition-all duration-700"
            style={{ backgroundColor: previewTheme.bg }}
          >
            <div
              className="w-14 h-14 rounded-full flex items-center justify-center text-lg font-display font-bold mb-4 transition-colors duration-500"
              style={{ backgroundColor: previewTheme.card, color: previewTheme.accent }}
            >
              A
            </div>
        ...
        <div className="space-y-0">
          {faqs.map((faq, i) => (
            <div key={i} className="faq-item border-b border-white/5" style={{ y: 25, opacity: 0 }}>
              ...
              <div
                className="overflow-hidden transition-all duration-400 ease-out-quart"
      ...
      <section className="relative px-6 py-32 md:py-48 max-w-3xl mx-auto text-center">
        <div className="cta-block">
          ...
          <div className="flex items-center justify-center gap-3 flex-wrap">
            <Link href="/auth/signup" className="group inline-flex items-center gap-2 rounded-full bg-gold text-ink px-6 py-3 text-caption font-semibold hover:opacity-90 transition-all active:scale-[0.97]">Start your page<ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" /></Link>
